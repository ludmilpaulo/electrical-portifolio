      ...
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <nav className="hidden md:flex gap-6 items-center">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 bg-brand-600 text-white px-4 py-2 rounded-xl shadow-soft"
          >
            <PhoneCall className="w-4 h-4" /> {phone || 'Call us'}
          </a>
        </nav>
      ...
      {open && (
        <div className="md:hidden border-t border-slate-200 bg-white">
          <div className="container px-4 py-3 flex flex-col gap-3">
            {links.map((l) => (
              <Link key={l.href} href={l.href} className="text-slate-700" onClick={() => setOpen(false)}>
                {l.label}
              </Link>
            ))}
            <a
              href={tel ? `tel:${tel}` : undefined}
              className="inline-flex items-center gap-2 bg-brand-600 text-white px-4 py-2 rounded-xl"
            >
              <PhoneCall className="w-4 h-4" /> {phone || 'Call us'}
            </a>
          </div>
